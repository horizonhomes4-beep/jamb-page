...
function mcDefaultPlans(){
  ...
  prices.forEach((price,i)=>{
    const months=i+1;
    out[`jamb-${months}m`]={
      label:`JAMB ${months} Month${months>1?'s':''}`,
      months,price,discount:discounts[i],discountPercent:discounts[i],
      currency:'NGN',description:`JAMB learning access for ${months} month${months>1?'s':''}.`
    };
  });
  return out;
}
...
const mcSession={
  ...
  setStudent(id,session=null){
    if(id) sessionStorage.setItem('mc_jamb_student_id',id);
    if(session) sessionStorage.setItem('mc_jamb_student_session',JSON.stringify(session));
  },
  clear(){sessionStorage.removeItem('mc_jamb_student_id');sessionStorage.removeItem('mc_jamb_student_session');}
};
